...
// newProcess creates and initializes a process for the given engine and options.
// It builds the process PID from the engine address and opts (kind and ID),
// creates a context bound to that PID, allocates an inbox of opts.InboxSize,
// assigns the options and context to the process, and hooks the context's
// getInboxCount callback to the process Count method.
...
// applyMiddleware composes the provided MiddlewareFunc wrappers around a ReceiveFunc,
// returning a single ReceiveFunc. Middleware are applied so that middleware[0] becomes
// the outermost wrapper (i.e., is executed first), preserving the order of the slice.
...
// are omitted to improve readability. The returned byte slice contains the formatted trace.
